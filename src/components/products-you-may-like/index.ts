...
export default class ProductsYouMayLike extends LitElement {
  static properties = {
    config: { type: Object },
  };
  ...
  private getTitle() {
    ...
    return title?.[document.documentElement.lang] || title?.ar || 'منتجات قد تعجبك';
  }

  render() {
    const section = document.createElement('section');
    section.className = 's-block s-block-products-you-may-like';

    const container = document.createElement('div');
    container.className = 'container';

    const title = document.createElement('h2');
    title.className = 's-block-products-you-may-like__title';
