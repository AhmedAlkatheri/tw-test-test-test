...
export default class RecommendedProducts extends LitElement {
  static properties = {
    config: { type: Object },
  };
  ...
  private getTitle() {
    ...
    return title?.[document.documentElement.lang] || title?.ar || 'منتجات نرشحها لك';
  }

  render() {
    const section = document.createElement('section');
    section.className = 's-block s-block-recommended-products';

    const container = document.createElement('div');
    container.className = 'container';

    const title = document.createElement('h2');
    title.className = 's-block-recommended-products__title';
